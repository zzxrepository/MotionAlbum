...
- 目录树显示每个目录连同下级目录的媒体总数；侧边栏默认保持紧凑，可像 VS Code 一样沿整条右边界平滑拖动。拖动期间提供缩放光标、边界高亮和实时宽度提示，释放后记住宽度，双击边界可复位；窗口变窄时会优先保障图库空间。
...
目录树只列出含有受支持媒体的目录及其必要的父目录，空目录不会占用侧边栏空间。每个含媒体的目录都能展开：子目录排在前面，本层图片和独立视频排在后面；已经与 Apple Live Photo 配对的 MOV 仍归属于对应图片，不会重复列出。目录行右侧数字表示该目录分支（含下级目录）的媒体总数，悬停提示会同时说明本层数量。
