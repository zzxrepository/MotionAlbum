...
- 将用户选择的大目录作为图库根目录建立索引，并在侧边栏像资源管理器一样逐层展开目录和其中的媒体文件；单击文件会立即切换到所在目录并在右侧定位该照片。
- 默认只展示当前选中目录本层的照片；需要汇总时可选择“包含所选目录的子目录”，切换目录无需重新扫描。
- 目录树显示每个目录连同下级目录的媒体总数和实际占用空间；侧边栏默认保持紧凑，可像 VS Code 一样沿整条右边界平滑拖动。拖动使用稳定的窗口坐标，不会因分隔条自身移动而抖动；释放后记住宽度，双击边界可复位，窗口变窄时会优先保障图库空间。
...
2. 在侧边栏展开目录树并单击一个目录，右侧只展示这个目录本层的照片和视频；目录还能继续展开到受支持的图片或独立视频文件，单击文件会在右侧立即定位，目录中的其他类型文件不会列入图库。
...
目录树只列出含有受支持媒体的目录及其必要的父目录，空目录不会占用侧边栏空间。每个含媒体的目录都能展开：子目录排在前面，本层图片和独立视频排在后面；已经与 Apple Live Photo 配对的 MOV 仍归属于对应图片，不会重复列出。目录行右侧采用“媒体数量 · 占用空间”，两项均统计该目录及下级目录；Apple Live Photo 的图片和配套 MOV 都计入空间，悬停提示会同时说明本层数据。
